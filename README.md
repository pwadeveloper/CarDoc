...
CarDoc is an installable progressive web app. A service worker (generated by
`vite-plugin-pwa`) precaches the application shell, so after one online visit
the app opens and runs with no connection.

Install it from the sidebar's **Install app** button on Chromium browsers. iOS
Safari does not implement `beforeinstallprompt`, so the button never appears
there; install through Share → Add to Home Screen instead. Long-pressing the
installed icon offers shortcuts to Diagnostics, Service journal and Owner's
manual.

What works with no connection:

- The full reference set — systems, the 31 OBD-II code entries, companion
  guides and the 40 manual topic shortcuts — because it is bundled into the
  JavaScript at build time rather than fetched.
- The service journal and vehicle profile, which are local-storage first.
- The translated manual reader.
- Typography. Google Fonts stylesheets and files are runtime-cached, so text
  renders in DM Sans and Manrope offline instead of falling back.

What needs a connection, and says so rather than failing silently:

- AI answers. `/api/ask` is never cached; with no network the assistant
  returns the labeled built-in answer instead.
- Vercel Blob sync. `/api/journal` is never cached, so cloud data is never
  served stale.
- The factory PDFs. At roughly 5 MB each they are excluded from the precache
  and cached only once actually opened, up to four editions.

A journal entry saved offline is queued in the `cardoc-journal-pending`
local-storage key and pushed on the next `online` event, so the sync badge
reads "Saved here — syncs when online" rather than stalling on "Saved
locally". The queue holds the whole journal document, so the newest write
supersedes any earlier queued one; there is no field-level merge, and a change
made on a second device while this one was offline is overwritten rather than
combined. A payload the server rejects (4xx) or a deployment with no Blob
store (503) clears the queue instead of retrying forever.

Service workers do not run on the Vite dev server; `devOptions` is disabled so
development keeps live reload. Test offline behaviour against `npm run
preview`, which serves the real build. Icons are generated by `npm run icons`
from the brand mark and committed, so a normal build needs no canvas step.
...
Original PDF illustrations were rendered and visually checked (hood release, jacking points and fuse boxes). Offline behaviour was verified in headless Chromium against `npm run preview`: the service worker activates, the app reloads and renders with the preview server stopped, cached fonts still resolve, and an app-shortcut deep link opens the right page. Installation itself was not confirmed on real iOS or Android hardware. Live OpenAI output has not been tested without credentials; mocked provider behavior and the unconfigured local endpoint are tested.
